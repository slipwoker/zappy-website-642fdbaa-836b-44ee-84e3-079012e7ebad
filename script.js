document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, rule, ruleValue) {
      const label = field.closest('[class*="field"], [class*="group"], label, div')
        ? (field.closest('label') || field.previousElementSibling || field.parentElement.querySelector('label'))
        : null;
      const fieldName = label ? label.textContent.trim().replace('*', '').trim()
    ...
    function validateField(field) {
      ...
      for (let i = 0; i < rules.length; i++) {
        ...
        if (validators[ruleName]) {
          const passed = ruleValue
            ? validators[ruleName](value, ruleValue)
            : validators[ruleName](value);

          if (!passed) {
            showError(field, getErrorMessage(field, ruleName, ruleValue));
            isValid = false;
            break;
          }
        }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "נילי ברקוביץ' משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '642fdbaa-836b-44ee-84e3-079012e7ebad',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '642fdbaa-836b-44ee-84e3-079012e7ebad';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '642fdbaa-836b-44ee-84e3-079012e7ebad';
